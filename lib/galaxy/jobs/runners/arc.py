...
from galaxy.jobs.runners.util.pyarcrest.http import HTTPClient
...
from galaxy.jobs.runners.util.pyarcrest.common import getNullLogger
...
class ArcRESTJobRunner(AsynchronousJobRunner):
    ...
    """ Create job script and submit it to arc - set arc cluster variable """
    ...
    self.arc.set_job_cluster(job_destination.params["arc_url"])

    """ Prepare and submit job to arc """
    self.prepare_job(job_wrapper, self.arcjob)
    arcjobs = {0:self.arcjob}

    token = job_wrapper.get_job().user.get_oidc_tokens(self.provider_backend)['access']
    arc_url = job_wrapper.job_destination.params['arc_url']

    # proxypath is ignored if you are using token
    self.arcrest = ARCRest.getClient(url=self.arc_url, version="1.1", token=token, impls={"1.1":ARCRest_1_1})
 
    # token parameter isn't necessary, unless there is a bug
    delegationID = self.arcrest.createDelegation()

    queue = self.arcjob.Queue

    bulkdesc = "<ActivityDescriptions>"
    for job in arcjobs:
        bulkdesc += job.descstr
    bulkdesc += "</ActivityDescriptions>"

    results = self.arcrest.createJobs(bulkdesc, queue, delegationID)
    
    if isinstance(results[0], ARCHTTPError):  # submission error
        ...
        errors = self.arcrest.uploadJobFiles(jobid, self.arcjob.inputFiles)
        if errors[0]:  # input upload error
            log.error("Job creation failure. No Response from ARC")
            job_wrapper.fail("Not submitted")
        else:  # successful input upload
            self.arc.job_mapping[galaxy_job_id]=arc_job_id
            # Create an object of AsynchronousJobState and add it to the monitor queue.
            ajs = AsynchronousJobState(files_dir=job_wrapper.working_directory, job_wrapper=job_wrapper, job_id=galaxy_job_id, job_destination=job_destination)
    ...
    def place_output_files(self, job_state, job_status_arc):
        ...
        arc_job_id = self.arc.job_mapping[job_state.job_id]
        ...
        try:
            ...
            with open(galaxy_workdir+"/"+arc_job_id+"/arc.out","r") as f:
                out_log = f.read()
            ...
            with open(galaxy_workdir+"/"+arc_job_id+"/arc.err","r") as f:
                err_log = f.read()
    ...
    def check_watched_item(self, job_state):
        ...
        self.arc_url = galaxy_job_wrapper.job_destination.params['arc_url']
        ...
        """ Get task from ARC """
        arc_job_id = self.arc.job_mapping[job_state.job_id]
        arc_job_state = self.job_actions(arc_job_id, "status")
        ...
        if arc_job_state:
            ...
            #log.error('Could not find status of job with galaxy id: ',job_id)
            #log.error(e)
            return None
        
        log.debug(f"Galaxy job ID: {job_state.job_id}  To ARC endpoint: {self.arc_url} Mapped job Status: {mapped_state} ARC-job status: {arc_job_state} ARC job ID: {arc_job_id}")
        self.arcrest = ARCRest.getClient(url=self.arc_url, version="1.1", token=self._get_token(galaxy_job_wrapper), impls={"1.1":ARCRest_1_1})
    ...
    def stop_job(self, job_wrapper):
        ...
        self.arc_url = job_wrapper.job_destination.params['arc_url']
        arc_job_id = ''
        ...
        try:
            arc_job_id = self.arc.job_mapping[job_id]
        except KeyError:
            log.debug(f'Could not find arc_job_id for stopping job {job_id}')
            return None
        ...
        if not(mapped_state == "Killed" or mapped_state == "Deleted" or mapped_state == "Finished"):
            ...
            try:
                ...
                waskilled = self.job_actions([arc_job_id],"kill")
                log.debug(f'>>>>>MAIKEN>>>>> killing job!')
            except Exception as e:
                log.debug(f'Job with ARC id: {arc_job_id} and Galaxy id: {job_id} was attempted to be killed by external request, but this did not succeed. Exception was: {e}')
    ...
    def prepare_job(self, job_wrapper, arcjob):
       ...
       <ActivityDescriptions>
        ...
        #log.debug(f'dir(galaxy_job): {dir(galaxy_job)}')
        job_input_params = {}
        ...
        arc_queue = str(job_destination.params["arc_queue"])
        ...
        """ TODO - the queue should be dynamically found for an endpoint - how is that handled in ARC? """
        queue = arc_queue
        sub_el = SubElement(resources,"QueueName")
        sub_el.text = arc_queue
